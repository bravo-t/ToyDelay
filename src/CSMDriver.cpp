...
double
CSMDriver::calcEffectiveCap(const SimResult& simResult, double timeStart, double timeEnd)
{
  if (simResult.empty()) {
    double totalCap = 0;
...
void
CSMDriver::updateCircuit(const SimResult& simResult, double timeStart, double timeEnd)
{
  double effCap = 0;
  if (simResult.empty()) {
    double totalCap = 0;
